...
header("HTTP/1.1 200 OK");
   ...
   $db = \Typecho\Db::get();
    switch($_POST['action']){
       case 'like':
           $like = $db->fetchRow($db->select()->from('table.bearhoney_friendcircle_data')->where('coid = ?', $_POST['coid']));
           if($user->hasLogin()){
        ...
        "msg" => '您已点过赞，请勿重复点赞！',
           ...
           if($like['likeUser'] == null){
            ...
            $db->query($db->update('table.bearhoney_friendcircle_data')->rows(array('likeUser' => $userName))->where('coid = ?', $_POST['coid']));
        ...
        "msg" => '您已点过赞，请勿重复点赞！',
               ...
               if($like['likeVisitor'] == null){
                ...
                $db->query($db->update('table.bearhoney_friendcircle_data')->rows(array('likeVisitor' => (int)$visitorNum))->where('coid = ?', $_POST['coid']));
        ...
        "likeUser" => $like['likeUser'],
           ...
           case 'delete':
               ...
               $coid = $_POST['coid'];
               $db->query($db->delete('table.bearhoney_friendcircle_data')->where('coid = ?', $coid));
