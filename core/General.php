...
class General
{
    public static function getCommentAction(){
    ...
    public static function getCircleAction(){
    ...
    public static function getPostAllTags($widget, $split = '', $default = NULL)
{
 ...
 $result[] .= '</div>';
    ...
    $chinese_word_count = mb_strlen($chinese_text, 'utf-8');
    
    // 计算英文字符数量
    $english_text = preg_replace("/[\x{4e00}-\x{9fa5}]/u", "", $text);
    $english_word_count = strlen($english_text);
    
    // 计算总字数
    $total_word_count = $chinese_word_count + $english_word_count;
    
    $reading_speed = 200;
    
    // 计算阅读时间（单位：分钟）
    $reading_time = ceil($total_word_count / $reading_speed);
 ...
 $curl = curl_init();
   ...
   //获取随机文章
    public static function getRandomPosts($cid){
$db = \Typecho\Db::get();
...
$html[] = '
<div class="col-sm-6">
                <div class="related-post-card" style="padding:20px">
...
return $result;
    ...
    public static function getWalls($t){
        $rand = mt_rand(1,7);
        $img = Helper::options()->themeUrl.'/assets/images/randPic/'.$rand.'.webp';
        return $img;
    }
...
if(!$val['hidden']){
    echo '
        ...
        <a class="previous-post" href="'.$val['permalink'].'" 
            style="background-image: url('.self::getWalls('next').')">
...
if(!$val['hidden']){
  ...
  <div class="previous">
        <a class="previous-post" href="'.$val['permalink'].'"
            style="background-image: url('.self::getWalls('prev').')" >
            <div class="d-flex w-100">
  ...
  echo ' <div class="previous">
        <a class="previous-post" style="background-image: url('.self::getWalls('prev').')" >
            <div class="d-flex w-100">
                <div class="previous-content">
                    <div class="arrow-text">上一篇</div>
